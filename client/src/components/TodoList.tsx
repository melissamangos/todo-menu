...
const ENERGY_META: Record<Todo["energyCost"], { color: string; bg: string; label: string; dot: string }> = {
  low:    { color: "var(--energy-low)",  bg: "var(--energy-low-bg)",  label: "Low energy",    dot: "#2dd4bf" },
  medium: { color: "var(--energy-med)",  bg: "var(--energy-med-bg)",  label: "Medium energy", dot: "#a78bfa" },
  high:   { color: "var(--energy-high)", bg: "var(--energy-high-bg)", label: "High energy",   dot: "#f472b6" },
};
  ...
  if (todos.length === 0) {
    return (
      <div style={{
        ...
        <p style={{ fontFamily: "var(--font-display)", fontStyle: "italic", fontSize: 18, color: "var(--slate-light)", marginBottom: 4 }}>
          Your menu is empty
        </p>
...
function TodoCard({ todo, index, onDelete }: { todo: Todo; index: number; onDelete: (id: string) => Promise<void> }) {
      ...
      <div style={{ display: "flex", alignItems: "flex-start", gap: 12 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 5 }}>
            {/* Timeslot */}
            <span className="chip" style={{ background: "var(--bg-elevated)", color: "var(--slate-light)", border: "1px solid var(--border-subtle)" }}>
